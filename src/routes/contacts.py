...
@router.get("/search/{contact_value}", response_model=List[ContactResponse])
async def get_contacts_by_field(
        contact_value: str,
        db: AsyncSession = Depends(get_db)
):
    contacts = await repository_contacts.get_by_field(contact_value, db)
    return contacts


@router.get("/birthday/next-week", response_model=List[ContactResponse])
async def get_birthday_next_week(db: AsyncSession = Depends(get_db)):
    contacts = await repository_contacts.birthday_week_contacts(db)
    return contacts
